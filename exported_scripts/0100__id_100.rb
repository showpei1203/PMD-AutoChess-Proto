...
class Scene_Battle < Scene_Base
  ...
  # ● 終了処理
  #--------------------------------------------------------------------------
  alias terminate_n01 terminate unless method_defined?(:terminate_n01)
  def terminate
    terminate_n01
    # 二刀流の持ち替え処理を戻す
  ...
  # ● 戦闘開始の処理
  #--------------------------------------------------------------------------
  alias process_battle_start_n01 process_battle_start unless method_defined?(:process_battle_start_n01)
  ...
  # ● フレーム更新
  #--------------------------------------------------------------------------
  alias update_n01 update unless method_defined?(:update_n01)
  def update
  ...
  # ● 敗北の処理
  #--------------------------------------------------------------------------
  alias process_defeat_n01 process_defeat unless method_defined?(:process_defeat_n01)
  def process_defeat
    for member in $game_party.members
      @spriteset.set_stand_by_action(member.actor?, member.index)
    end
  ...
  # ● 次のアクターのコマンド入力へ
  #--------------------------------------------------------------------------
  alias next_actor_n01 next_actor unless method_defined?(:next_actor_n01)
  def next_actor
    # 動けるキャラのみコマンドアクションを
  ...
  # ● 前のアクターのコマンド入力へ
  #--------------------------------------------------------------------------
  alias prior_actor_n01 prior_actor unless method_defined?(:prior_actor_n01)
  def prior_actor
    # 動けるキャラのみコマンドアクションを
    if @active_battler != nil && @active_battler.inputable? && @active_battler.actor?
  ...
  # ● 勝利の処理
  #--------------------------------------------------------------------------
  alias process_victory_n01 process_victory unless method_defined?(:process_victory_n01)
  def process_victory
  ...
  # ● 行動順序作成
  #--------------------------------------------------------------------------
  alias make_action_orders_n01 make_action_orders unless method_defined?(:make_action_orders_n01)
  ...
  # ● 戦闘行動の実行
  #--------------------------------------------------------------------------
  alias execute_action_n01 execute_action unless method_defined?(:execute_action_n01)
  def execute_action
...
class Sprite_Base < Sprite
  ...
  #--------------------------------------------------------------------------
  alias update_animation_n01 update_animation unless method_defined?(:update_animation_n01)
...
class Game_Battler
  ...
  # ● オブジェクト初期化
  #--------------------------------------------------------------------------
  alias initialize_n01 initialize unless method_defined?(:initialize_n01)
  def initialize
    initialize_n01
  ...
  # ● 通常攻撃によるダメージ計算  二刀流補正
  #--------------------------------------------------------------------------
  alias make_attack_damage_value_n01 make_attack_damage_value unless method_defined?(:make_attack_damage_value_n01)
  ...
  #--------------------------------------------------------------------------
  alias make_obj_damage_value_n01 make_obj_damage_value unless method_defined?(:make_obj_damage_value_n01)
  ...
  # ● ダメージの反映
  #--------------------------------------------------------------------------
  alias execute_damage_n01 execute_damage unless method_defined?(:execute_damage_n01)
  ...
  # ● スキルの効果適用
  #--------------------------------------------------------------------------
  alias skill_effect_n01 skill_effect unless method_defined?(:skill_effect_n01)
...
class Game_Party < Game_Unit
  ...
  #     actor_id : アクター ID
  #--------------------------------------------------------------------------
  alias add_actor_n01 add_actor unless method_defined?(:add_actor_n01)
  ...
  #     actor_id : アクター ID
  #--------------------------------------------------------------------------
  alias remove_actor_n01 remove_actor unless method_defined?(:remove_actor_n01)
