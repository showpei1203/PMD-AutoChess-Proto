# encoding: UTF-8
#===============================================================================
# ■ PMD AutoChess - v1.06.62a Gate 3 Floor-Depth Risk Windows Acceptance
#   TEST-ONLY
#-------------------------------------------------------------------------------
# Plain F5 on active Map090 Hunt. Read-only acceptance.
#===============================================================================
$imported={} unless defined?($imported)
$imported['PMD_AutoChess_Gate3FloorDepthRiskWindowsAcceptance_v10662a']=true

module PMD_AC
  GATE3_V10662A_LOG='PMD_GATE3_FloorDepthRisk_LATEST.log'
  class << self
    def gate3_v10662a_project_state
      bad=[];text=''
      begin
    ...
    def gate3_v10662a_current_floor
      ...
      max=s[:vxrd_max_floors_v10604].to_i
    ...
    def gate3_v10662a_run
      ...
      current=gate3_v10662a_current_floor
      ps=gate3_v10662a_project_state
    ...
    def gate3_v10662a_write(r)
      c=r[:current]||{};s=r[:static]||{};p=r[:project_state]||{}
      lines=[]
      lines << 'PMD AutoChess v1.06.62a Gate 3 Floor-Depth Risk Windows Acceptance TEST-ONLY'
      ...
      File.open(GATE3_V10662A_LOG,'wb'){|io|io.write(lines.join("\r\n")+"\r\n")}
      r
    rescue
      r
    end
...
class Scene_Map
  alias pmd_ac_v10662a_gate3_update update unless method_defined?(:pmd_ac_v10662a_gate3_update)
  alias pmd_ac_v10662a_gate3_terminate terminate unless method_defined?(:pmd_ac_v10662a_gate3_terminate)
  def gate3_v10662a_dispose
    if @gate3_v10662a_sprite!=nil
      begin;b=@gate3_v10662a_sprite.bitmap;b.dispose if b!=nil && !b.disposed?;rescue;end
      begin;@gate3_v10662a_sprite.dispose unless @gate3_v10662a_sprite.disposed?;rescue;end
    end
    @gate3_v10662a_sprite=nil;@gate3_v10662a_frames=0
  rescue
  end
  def gate3_v10662a_show(r)
    gate3_v10662a_dispose
    ...
    b.draw_text(16,Graphics.height-58,Graphics.width-32,24,'LOG: '+PMD_AC::GATE3_V10662A_LOG,1)
    @gate3_v10662a_sprite=s;@gate3_v10662a_frames=360
  rescue
  end
  def update
    pmd_ac_v10662a_gate3_update
    if @gate3_v10662a_frames.to_i>0
      @gate3_v10662a_frames-=1
      gate3_v10662a_dispose if @gate3_v10662a_frames.to_i<=0
    end
    plain=false
    begin
    ...
    if plain
      r=PMD_AC.gate3_v10662a_run
      PMD_AC.gate3_v10662a_write(r)
      gate3_v10662a_show(r)
    end
  rescue
    pmd_ac_v10662a_gate3_update
  end
  def terminate
    gate3_v10662a_dispose
    pmd_ac_v10662a_gate3_terminate
  end
end
